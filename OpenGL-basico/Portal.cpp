...
void Portal::draw()
{
    ...
    DrawOBJ(portalModel, portalTexture);
    glPopMatrix();
}
...
Portal::Portal(Vector3 coordinates, const char *objPath, const char *texPath) : GameObject(coordinates)
{
    glShadeModel(GL_SMOOTH);
    LoadOBJ(objPath, portalModel);
    portalTexture = LoadTexture(texPath);
}
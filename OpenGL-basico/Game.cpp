...
void Game::drawTextGame(const char *text, float x, float y, SDL_Color color, bool useCache)
{
    ...
    glVertex2f(x + textWidthCache, oglY);
    glTexCoord2f(1.0f, 0.0f);
    glVertex2f(x + textWidthCache, oglY + textHeightCache);
    glTexCoord2f(0.0f, 0.0f);
    glVertex2f(x, oglY + textHeightCache);
    glEnd();
...
void Game::renderHUD()
{
    ...
    float x = 10.0f;
    float y = 10.0f;
    drawTextGame(scoreText.c_str(), x, y, textColor, false);

    std::string timeText = "Time: " + std::to_string((int)currentTime);
    x = 200.0f;
    y = 10.0f;
    drawTextGame(timeText.c_str(), x, y, textColor, false);
...
void Game::processKey(const SDL_Event &event)
{
    if (this->gameState == WAITING_FOR_INPUT || this->gameState == PROCESSING)
    {
        ...
        {
        case SDLK_SPACE:
            if (this->willWormEnterPortal(this->worm->getOrientationForward()))
            {
                this->gameState = GameState::YOU_WIN;
            }
            else if (this->willWormTouchLava(this->worm->getOrientationForward()))
            {
                this->gameState = GameState::YOU_LOSE;
            }
            else if (this->willWormEatApple(this->worm->getOrientationForward()))
            {
                this->growWorm();
        ...
        case SDLK_UP:
            ...
            if (this->willWormEnterPortal(option))
            {
                this->gameState = GameState::YOU_WIN;
            }
            else if (this->willWormTouchLava(option))
            {
                this->gameState = GameState::YOU_LOSE;
            }
            else if (this->willWormEatApple(option))
            {
...
bool Game::willWormTouchLava(Vector3 orientation)
{
    if (!worm || !grid)
        return false;
    Vector3 nextPos = worm->calculateWormPotentialNextPosition(orientation);
    CubeGridElement *slot = grid->at(nextPos);
    return slot != nullptr && slot->hasLava();
}
...
bool Game::willWormEnterPortal(Vector3 orientation)
{
    if (!worm || !grid)
        return false;
    Vector3 nextPos = worm->calculateWormPotentialNextPosition(orientation);
    CubeGridElement *slot = grid->at(nextPos);
    return slot != nullptr && slot->hasPortal();
}
